    ...
    success: function (data) {
    	...
    	for (i=0; i<Object.keys(data.events).length; i++) {
    		...
    		if (data.events[i].startTimestamp >= moment().unix() - (3600*24) && data.events[i].startTimestamp <= todayEndTimestamp && data.events[i].status.description != 'Postponed' && data.events[i].status.description != 'Abandoned') {
	            ...
	            $(`.matches > .match#${data.events[i].id}`).append(`<div class='home'><img class='team_logo' src='https://api.sofascore.app/api/v1/team/${data.events[i].homeTeam.id}/image'><p class='home_team'>${data.events[i].homeTeam.shortName}</p>${RedCardHome}<p class='home_score'>${homeScore}</p></div><div class='away'><img class='team_logo' src='https://api.sofascore.app/api/v1/team/${data.events[i].awayTeam.id}/image'><p class='away_team'>${data.events[i].awayTeam.shortName}</p>${RedCardAway}<p class='away_score'>${awayScore}</p></div>`);
