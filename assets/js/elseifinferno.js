...
$.ajax({
    ...
    success: function (data) {
    	...
    	for (i=0; i<Object.keys(data.events).length; i++) {
    		...
    		if (data.events[i].startTimestamp >= moment().unix() - (3600*24) && data.events[i].startTimestamp <= todayEndTimestamp && data.events[i].status.description != 'Postponed' && data.events[i].status.description != 'Abandoned' && data.events[i].status.description != 'Canceled') {
    			let flag;
	            let status;
	            let tournament;
	            let time;
